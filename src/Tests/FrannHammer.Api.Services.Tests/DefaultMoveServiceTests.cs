...
namespace FrannHammer.Api.Services.Tests
{
    ...
    public class DefaultMoveServiceTests
    {
        ...
        public void DetailedMovesContainExpectedProperties()
        {
            ...
            Assert.That(firstMove.MoveProperties.Any(data => data.MoveAttributes.Any(d => d.Name.Equals(Hitbox1Key))),
                $"{nameof(firstMove)} does not contain {Hitbox1Key}: {nameof(firstMove)}");
            Assert.That(firstMove.MoveProperties.Any(data => data.MoveAttributes.Any(d => d.Name.Equals(Hitbox2Key))),
                $"{nameof(firstMove)} does not contain {Hitbox2Key}: {nameof(firstMove)}");
            Assert.That(firstMove.MoveProperties.Any(data => data.MoveAttributes.Any(d => d.Name.Equals(Hitbox3Key))),
                $"{nameof(firstMove)} does not contain {Hitbox3Key}: {nameof(firstMove)}");
            Assert.That(firstMove.MoveProperties.Any(data => data.MoveAttributes.Any(d => d.Name.Equals(Hitbox4Key))),
                $"{nameof(firstMove)} does not contain {Hitbox4Key}: {nameof(firstMove)}");
            Assert.That(firstMove.MoveProperties.Any(data => data.MoveAttributes.Any(d => d.Name.Equals(Hitbox5Key))),
                $"{nameof(firstMove)} does not contain {Hitbox5Key}: {nameof(firstMove)}");
            Assert.That(firstMove.MoveProperties.Any(data => data.MoveAttributes.Any(d => d.Name.Equals(RawValueKey))),
                $"{nameof(firstMove)} does not contain {RawValueKey}: {nameof(firstMove)}");
            Assert.That(firstMove.MoveProperties.Any(data => data.MoveAttributes.Any(d => d.Name.Equals(NotesKey))),
        ...
        public void NullValueForSpecificPropertyReturnsNullValueWhenRetrievingSpecificPropertyOfMoves()
        {
            ...
            const int expectedNumberOfKeysForEachResult = 5;
